using System;
using System.Linq;
using Microsoft.Xna.Framework;
using Microsoft.Xna.Framework.Graphics;
using Terraria;
using Terraria.ID;
using Terraria.ModLoader;

namespace StingermanMod.Projectiles
{
    public class SawBlade : ModProjectile
    {
        public bool IsDivisible(int x, int n)
        {
            return (x % n) == 0;
        }
        public static int AndTheBleedDamage(ref int bleedingDamageModifer)
        {
            float baseDamage = 0.5f;
            baseDamage *= (float)bleedingDamageModifer;
            bleedingDamageModifer = (int)baseDamage + bleedingDamageModifer;
            return bleedingDamageModifer;
        }
        public override void SetStaticDefaults()
        {
            DisplayName.SetDefault("Sawblade");
        }
        public override void SetDefaults()
        {
            projectile.width = 32;
            projectile.height = 32;
            projectile.penetrate = 5;
            projectile.friendly = true;
            projectile.magic = true;
            projectile.timeLeft = 60;
            projectile.light = 0.05f;
            projectile.usesLocalNPCImmunity = true;
            projectile.localNPCHitCooldown = 20;
            projectile.ignoreWater = NPC.downedFishron;
            projectile.tileCollide = !Main.hardMode;
            if (NPC.downedSlimeKing == true)
            {
                projectile.penetrate = projectile.penetrate + 3;
                projectile.timeLeft = projectile.timeLeft + 57;
                projectile.light = projectile.light + 0.035f;
                projectile.localNPCHitCooldown = projectile.localNPCHitCooldown - 1;
            }
            if (NPC.downedBoss1 == true)
            {
                projectile.penetrate = projectile.penetrate + 3;
                projectile.timeLeft = projectile.timeLeft + 57;
                projectile.light = projectile.light + 0.035f;
                projectile.localNPCHitCooldown = projectile.localNPCHitCooldown - 1;
            }
            if (NPC.downedBoss2 == true)
            {
                projectile.penetrate = projectile.penetrate + 3;
                projectile.timeLeft = projectile.timeLeft + 57;
                projectile.light = projectile.light + 0.035f;
                projectile.localNPCHitCooldown = projectile.localNPCHitCooldown - 1;
            }
            if (NPC.downedBoss3 == true)
            {
                projectile.penetrate = projectile.penetrate + 3;
                projectile.timeLeft = projectile.timeLeft + 57;
                projectile.light = projectile.light + 0.035f;
                projectile.localNPCHitCooldown = projectile.localNPCHitCooldown - 1;
            }
            if (NPC.downedQueenBee == true)
            {
                projectile.penetrate = projectile.penetrate + 3;
                projectile.timeLeft = projectile.timeLeft + 57;
                projectile.light = projectile.light + 0.035f;
                projectile.localNPCHitCooldown = projectile.localNPCHitCooldown - 1;
            }
            if (Main.hardMode == true)
            {
                projectile.penetrate = projectile.penetrate + 3;
                projectile.timeLeft = projectile.timeLeft + 57;
                projectile.light = projectile.light + 0.035f;
                projectile.localNPCHitCooldown = projectile.localNPCHitCooldown - 1;
            }
            if (StingermanMod.mechTwinsFirstKill == true)
            {
                projectile.penetrate = projectile.penetrate + 3;
                projectile.timeLeft = projectile.timeLeft + 57;
                projectile.light = projectile.light + 0.035f;
                projectile.localNPCHitCooldown = projectile.localNPCHitCooldown - 1;
            }
            if (StingermanMod.mechDestroyerFirstKill == true)
            {
                projectile.penetrate = projectile.penetrate + 3;
                projectile.timeLeft = projectile.timeLeft + 57;
                projectile.light = projectile.light + 0.035f;
                projectile.localNPCHitCooldown = projectile.localNPCHitCooldown - 1;
            }
            if (StingermanMod.mechSkeletronPrimeFirstKill == true)
            {
                projectile.penetrate = projectile.penetrate + 3;
                projectile.timeLeft = projectile.timeLeft + 57;
                projectile.light = projectile.light + 0.035f;
                projectile.localNPCHitCooldown = projectile.localNPCHitCooldown - 1;
            }
            if (NPC.downedPlantBoss == true)
            {
                projectile.penetrate = projectile.penetrate + 3;
                projectile.timeLeft = projectile.timeLeft + 57;
                projectile.light = projectile.light + 0.035f;
                projectile.localNPCHitCooldown = projectile.localNPCHitCooldown - 1;
            }
            if (NPC.downedGolemBoss == true)
            {
                projectile.penetrate = projectile.penetrate + 3;
                projectile.timeLeft = projectile.timeLeft + 57;
                projectile.light = projectile.light + 0.035f;
                projectile.localNPCHitCooldown = projectile.localNPCHitCooldown - 1;
            }
            if (NPC.downedFishron == true)
            {
                projectile.penetrate = projectile.penetrate + 3;
                projectile.timeLeft = projectile.timeLeft + 57;
                projectile.light = projectile.light + 0.035f;
                projectile.localNPCHitCooldown = projectile.localNPCHitCooldown - 1;
            }
            if (StingermanMod.ancientCultistFirstKill == true)
            {
                projectile.penetrate = projectile.penetrate + 3;
                projectile.timeLeft = projectile.timeLeft + 57;
                projectile.light = projectile.light + 0.035f;
                projectile.localNPCHitCooldown = projectile.localNPCHitCooldown - 1;
            }
            if (NPC.downedMoonlord == true)
            {
                projectile.penetrate = projectile.penetrate + 3;
                projectile.timeLeft = projectile.timeLeft + 57;
                projectile.light = projectile.light + 0.035f;
                projectile.localNPCHitCooldown = projectile.localNPCHitCooldown - 1;
            }
            if (NPC.downedFrost == true)
            {
                projectile.penetrate = projectile.penetrate + 3;
                projectile.timeLeft = projectile.timeLeft + 57;
                projectile.light = projectile.light + 0.035f;
                projectile.localNPCHitCooldown = projectile.localNPCHitCooldown - 1;
            }
            if (NPC.downedGoblins == true)
            {
                projectile.penetrate = projectile.penetrate + 3;
                projectile.timeLeft = projectile.timeLeft + 57;
                projectile.light = projectile.light + 0.035f;
                projectile.localNPCHitCooldown = projectile.localNPCHitCooldown - 1;
            }
            if (NPC.downedPirates == true)
            {
                projectile.penetrate = projectile.penetrate + 3;
                projectile.timeLeft = projectile.timeLeft + 57;
                projectile.light = projectile.light + 0.035f;
                projectile.localNPCHitCooldown = projectile.localNPCHitCooldown - 1;
            }
            if (NPC.downedMartians == true)
            {
                projectile.penetrate = projectile.penetrate + 3;
                projectile.timeLeft = projectile.timeLeft + 57;
                projectile.light = projectile.light + 0.035f;
                projectile.localNPCHitCooldown = projectile.localNPCHitCooldown - 1;
            }
            if (NPC.downedHalloweenKing == true)
            {
                projectile.penetrate = projectile.penetrate + 3;
                projectile.timeLeft = projectile.timeLeft + 57;
                projectile.light = projectile.light + 0.035f;
                projectile.localNPCHitCooldown = projectile.localNPCHitCooldown - 1;
            }
            if (NPC.downedChristmasIceQueen == true)
            {
                projectile.penetrate = projectile.penetrate + 3;
                projectile.timeLeft = projectile.timeLeft + 57;
                projectile.light = projectile.light + 0.035f;
                projectile.localNPCHitCooldown = projectile.localNPCHitCooldown - 1;
            }
        }
        public override bool OnTileCollide(Vector2 oldVelocity)
        {
            projectile.penetrate--;
            if (projectile.penetrate <= 0)
            {
                projectile.Kill();
            }
            Collision.HitTiles(projectile.position + projectile.velocity, projectile.velocity, projectile.width, projectile.height);
            Main.PlaySound(SoundID.NPCHit4, projectile.position);
            if (NPC.downedSlimeKing)
            {
                if (projectile.velocity.X != oldVelocity.X)
                {
                    projectile.velocity.X = -oldVelocity.X;
                }
                if (projectile.velocity.Y != oldVelocity.Y)
                {
                    projectile.velocity.Y = -oldVelocity.Y;
                }
            }
            else
            {
                projectile.Kill();
            }
            return false;
        }
        public override void AI()
        {
            if (NPC.downedBoss1)
            {
                float num132 = (float)Math.Sqrt((double)(projectile.velocity.X * projectile.velocity.X + projectile.velocity.Y * projectile.velocity.Y));
                float num133 = projectile.localAI[0];
                projectile.rotation += (float)projectile.direction * 15.0f;
                if (num133 == 0f)
                {
                    projectile.localAI[0] = num132;
                    num133 = num132;
                }
                float num134 = projectile.position.X;
                float num135 = projectile.position.Y;
                float num136 = 300f;                    //Homing range value.
                bool flag3 = false;
                int num137 = 0;
                if (StingermanMod.mechSkeletronPrimeFirstKill)      //Just changes the range of the projectile currently.
                {
                    num136 = 600f;
                }
                if (projectile.ai[1] == 0f)
                {
                    for (int num138 = 0; num138 < 200; num138++)
                    {
                        if (Main.npc[num138].CanBeChasedBy(this, false) && (projectile.ai[1] == 0f || projectile.ai[1] == (float)(num138 + 1)))
                        {
                            float num139 = Main.npc[num138].position.X + (float)(Main.npc[num138].width / 2);
                            float num140 = Main.npc[num138].position.Y + (float)(Main.npc[num138].height / 2);
                            float num141 = Math.Abs(projectile.position.X + (float)(projectile.width / 2) - num139) + Math.Abs(projectile.position.Y + (float)(projectile.height / 2) - num140);
                            if (StingermanMod.mechSkeletronPrimeFirstKill)
                            {
                                if (num141 < num136)
                                {
                                    num136 = num141;
                                    num134 = num139;
                                    num135 = num140;
                                    flag3 = true;
                                    num137 = num138;
                                }
                            }
                            else
                            {
                                if (num141 < num136 && Collision.CanHit(new Vector2(projectile.position.X + (float)(projectile.width / 2), projectile.position.Y + (float)(projectile.height / 2)), 1, 1, Main.npc[num138].position, Main.npc[num138].width, Main.npc[num138].height))
                                {
                                    num136 = num141;
                                    num134 = num139;
                                    num135 = num140;
                                    flag3 = true;
                                    num137 = num138;
                                }
                            }
                        }
                    }
                    if (flag3)
                    {
                        projectile.ai[1] = (float)(num137 + 1);
                    }
                    flag3 = false;
                }
                if (projectile.ai[1] > 0f)
                {
                    int num142 = (int)(projectile.ai[1] - 1f);
                    if (Main.npc[num142].active && Main.npc[num142].CanBeChasedBy(this, true) && !Main.npc[num142].dontTakeDamage)
                    {
                        float num143 = Main.npc[num142].position.X + (float)(Main.npc[num142].width / 2);
                        float num144 = Main.npc[num142].position.Y + (float)(Main.npc[num142].height / 2);
                        if (Math.Abs(projectile.position.X + (float)(projectile.width / 2) - num143) + Math.Abs(projectile.position.Y + (float)(projectile.height / 2) - num144) < 1000f)
                        {
                            flag3 = true;
                            num134 = Main.npc[num142].position.X + (float)(Main.npc[num142].width / 2);
                            num135 = Main.npc[num142].position.Y + (float)(Main.npc[num142].height / 2);
                        }
                    }
                    else
                    {
                        projectile.ai[1] = 0f;
                    }
                }
                if (!projectile.friendly)
                {
                    flag3 = false;
                }
                if (flag3)
                {
                    float num145 = num133;
                    Vector2 vector10 = new Vector2(projectile.position.X + (float)projectile.width * 0.5f, projectile.position.Y + (float)projectile.height * 0.5f);
                    float num146 = num134 - vector10.X;
                    float num147 = num135 - vector10.Y;
                    float num148 = (float)Math.Sqrt((double)(num146 * num146 + num147 * num147));
                    num148 = num145 / num148;
                    num146 *= num148;
                    num147 *= num148;
                    int num149 = 8;
                    projectile.velocity.X = (projectile.velocity.X * (float)(num149 - 1) + num146) / (float)num149;
                    projectile.velocity.Y = (projectile.velocity.Y * (float)(num149 - 1) + num147) / (float)num149;
                }
            }
        }
        public override void Kill(int timeLeft)
        {
            Player player = Main.player[projectile.owner];
            Main.PlaySound(SoundID.Item14, projectile.position);
            Dust.NewDust(projectile.position + projectile.velocity, projectile.width, projectile.height, 186, projectile.velocity.X * -0.5f, projectile.velocity.Y * -0.5f);
            if (player.statMana < player.statManaMax2)
            {
                if (StingermanMod.lunarPillarNebulaFirstKill)
                {
                    int manaHealMod = 5;
                    if (StingermanMod.mechTwinsFirstKill || NPC.downedMoonlord)
                    {
                        manaHealMod = 7;
                    }
                    else if (StingermanMod.mechTwinsFirstKill && NPC.downedMoonlord)
                    {
                        manaHealMod = 8;
                    }
                    player.statMana += manaHealMod;
                }
            }
        }
        public override void OnHitNPC(NPC target, int damage, float knockBack, bool crit)
        {
            projectile.penetrate--;
            Player player = Main.player[projectile.owner];
            if (target.HasBuff(BuffID.Bleeding))
            {
                AndTheBleedDamage(ref projectile.damage);
            }
            if (StingermanMod.bloodMoonSurvivalist)
            {
                target.AddBuff(BuffID.Bleeding, 300, false);
            }
            if ((NPC.downedQueenBee && !NPC.downedPlantBoss) || (!NPC.downedQueenBee && NPC.downedPlantBoss))
            {
                player.AddBuff(BuffID.Honey, 120, false);
            }
            else
            {
                if (IsDivisible(projectile.penetrate, 5))
                {
                    player.statLife += 1;
                }
            }
            if (StingermanMod.eaterOfWorldsFirstKill)
            {
                target.AddBuff(BuffID.Slow, 100, false);
            }
            if (StingermanMod.brainOfCthulhuFirstKill)
            {
                target.AddBuff(BuffID.Confused, 200, false);
            }
            if (Main.hardMode && NPC.downedSlimeKing)
            {
                target.AddBuff(BuffID.OnFire, 300, false);
            }
            if (NPC.downedChristmasIceQueen)
            {
                target.AddBuff(BuffID.Frostburn, 600, false);
            }
            if (NPC.downedHalloweenKing)
            {
                target.AddBuff(BuffID.Burning, 300, false);
            }
            if (StingermanMod.lunarPillarVortexFirstKill)
            {
                target.AddBuff(164, 300, false);
            }
            if (projectile.penetrate <= 0)
            {
                projectile.Kill();
            }
        }
    }
}